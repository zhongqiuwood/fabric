...
func TestConnection_Correct(t *testing.T) {
	...
	var err error
	if TLSEnabled(true) {
		tmpConn, err = NewClientConnectionWithAddress(viper.GetString("peer.cliaddress"), true, true, InitTLSForPeer())
	}
	tmpConn, err = NewClientConnectionWithAddress(viper.GetString("peer.cliaddress"), true, false, nil)
	if err != nil {
