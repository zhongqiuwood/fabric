...
func TestBucketNodeComputeHash(t *testing.T) {
	...
	childKey3 := newBucketKey(conf, 3, 21)
	...
	childKey2 := newBucketKey(conf, 3, 20)
...
func TestBucketNodeMerge(t *testing.T) {
	...
	dbBucketNode := newBucketNode(newBucketKey(conf, 2, 7))
...
func TestBucketNodeMarshalUnmarshal(t *testing.T) {
	...
	deserializedBucketNode := unmarshalBucketNode(newBucketKey(conf, 2, 7), serializedBytes)
