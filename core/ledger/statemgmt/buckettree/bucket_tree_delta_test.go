...
func TestBucketTreeDeltaBasic(t *testing.T) {
	...
	b2 := bucketTreeDelta.getOrCreateBucketNode(newBucketKey(conf, 2, 2))
	b3 := bucketTreeDelta.getOrCreateBucketNode(newBucketKey(conf, 2, 3))
	testutil.AssertContainsAll(t, bucketTreeDelta.getBucketNodesAt(2), []*bucketNode{b1, b2, b3})

	b4 := bucketTreeDelta.getOrCreateBucketNode(newBucketKey(conf, 1, 1))
	b5 := bucketTreeDelta.getOrCreateBucketNode(newBucketKey(conf, 1, 2))
	testutil.AssertContainsAll(t, bucketTreeDelta.getBucketNodesAt(1), []*bucketNode{b4, b5})

	b6 := bucketTreeDelta.getOrCreateBucketNode(newBucketKey(conf, 0, 1))
