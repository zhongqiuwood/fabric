...
func (proc *syncProcess) CurrentOffset() *protos.SyncOffset {

	data, err := proc.current.Byte()

	if err != nil {
		return nil
	}

	logger.Debugf("Current state offset <%+v>", proc.current)
	return &protos.SyncOffset{Data: data}
}
