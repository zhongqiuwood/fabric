...
import "github.com/abchain/fabric/core/db"

func fetchTrieNodeFromDB(odb *db.OpenchainDB, key *trieKey) (*trieNode, error) {
	stateTrieLogger.Debugf("Enter fetchTrieNodeFromDB() for trieKey [%s]", key)
	trieNodeBytes, err := odb.GetValue(db.StateCF, key.getEncodedBytes())
	if err != nil {
