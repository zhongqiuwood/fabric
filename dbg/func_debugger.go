...
func Int2string(v uint64) string {
...
func ChkErr(err error)  {
	if err != nil {
		log(2, ERROR, "Error: %s", err)
	}
}

func log(skip int, level int, format string, args ...interface{}) {
	if logger == nil {
		return
	}
	...
	funcName := strings.Split(f.Name(), "github.com/abchain/fabric/")

	format = "[" + goidstr() + "] " + action + "" + funcName[len(funcName) - 1] + ": " + format
	//format = goidstr() + "] " + action + "" + f.Name() + ": " + format
...
// dump call stack
func DumpStack() {
	logger.Infof("vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv")
